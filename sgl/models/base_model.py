...
class BaseSGAPModel(nn.Module):
    ...
    def preprocess(self, adj, feature, *args):
    ...
    @staticmethod
    def model_train(model, train_idx, labels, device, optimizer, loss_fn, accuracy):
        model.train()
        optimizer.zero_grad()
    ...
    def model_evaluate(model, val_idx, test_idx, labels, device, metric):
        ...
        acc_val = metric(val_output, labels[val_idx])
        acc_test = metric(test_output, labels[test_idx])
        
        return acc_val, acc_test 
    
    @staticmethod
    def model_mini_batch_train(model, train_idx, train_loader, labels, device, optimizer, loss_fn):
        model.train()
        correct_num = 0
        loss_train_sum = 0.
    ...
    @staticmethod
    @torch.no_grad()
    def model_mini_batch_evaluate(model, val_idx, val_loader, test_idx, test_loader, labels, device):
        model.eval()
