...
class GAugO(nn.Module):
    ...
    def preprocess(self, features, adj_matrix, device):
        ...
        adj_orig = sparse_mx_to_pyg_sparse_tensor(adj_matrix_sl).to_dense().to(device)
    ...
    def forward(self, adj_norm, adj_orig, features):
        adj_logits = self.ep_net(adj_norm, features)
...
class GAugM(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes, n_layers, gnn_type, rm_pct, add_pct, choose_idx, dropout=0.5, activation=F.relu, feat_norm='none', **kwargs):
