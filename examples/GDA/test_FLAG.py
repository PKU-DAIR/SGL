...
from sgl.models.homo.gda import FLAG
from sgl.tasks import NodeClassification

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description = "FLAG-Model.")
    parser.add_argument(
        "--device", type=int, default=0, help="gpu device id or cpu (-1)"
    )
    parser.add_argument(
        "--config_path", type=str, default="./configs/FLAG.yml", help="save path of the configuration file"
    )
    ...
    for seed in range(10):
        model = FLAG(in_dim=dataset.num_features, n_classes=dataset.num_classes, **model_kwargs)
